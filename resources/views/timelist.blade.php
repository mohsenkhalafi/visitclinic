...
<form id="frmm" action="timesho" method="post">
    <br class="inputs">
...
@foreach($shows as $timelist)
    <tr>
        <form action="timed/id={{$timelist->code}}/_xd={{$timelist->id}}" method="post">
            {{ csrf_field() }}
        <td class='text-left'>{{$timelist->fname}}  {{$timelist->lname}}</td>
        <td class='text-left'>{{$timelist->expert}}</td>
        <?php $sh=DB::table('schedule')
            ->leftjoin('drtime','drtime.id','=','schedule.drtimeid')
                ->select('schedule.*','drtime.*')
            ->where('schedule.drtimeid',$timelist->id)->get();
        compact('sh'); ?>


        <td class='text-left'><label for='se1' ></label>
            <select name="time" id="se1">
                {{$times=$timelist->fhour}}
               {{$times2=$timelist->thour}}
                <?php

                $myArray = array();
                foreach($sh as $timelists){
                    $myArray[] = $timelists->hour;}
                    


                $t1=date('H:i',strtotime($times));
                $t2=date('H:i',strtotime($times2));
...
//}
                    ...
                    $t1=strtotime("15 minutes",strtotime($t1));
                    echo $t5=date('H:i',$t1).'<br>'.'">';
$t7=$tf.'-'.$t5=date('H:i',$t1);
if($t4>$lenth){}
else{
if($t7==$myArray[$t4]){echo $t7.'-'.'رزرو شده';}
else{
   // echo $t7.'<br>';              //  var_dump  $myarray'<br>';
}
}
        ...
        <td class='text-left'><button type="submit">ثبت</button></td>
        </form>
    </tr>
    @endforeach
